...
def carregar_otimos(caminho_csv):
    ...
    with open(caminho, "r", encoding="utf-8") as f:
        ...
        for linha in leitor:
            ...
            if inst and not inst.lower().endswith(".dat"): inst += ".dat"
...
def executar():
    pasta = Path("Langevin")
    otimos = carregar_otimos("otimos_conhecidos.csv")
    resultados = []

    print(f"{'Instância':<15} | {'Ótimo':<8} | {'1-Shift':<8} | {'Gap %':<8} | {'Tempo(ms)':<8}")
    print("-" * 65)

    arquivos = sorted(pasta.glob("*.dat"))
    for arq in arquivos:
    ...
    with open("resultado_1shift.csv", "w", encoding="utf-8", newline="") as f:
