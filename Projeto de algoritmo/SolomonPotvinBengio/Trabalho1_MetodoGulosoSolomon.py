...
# padroniza o nome da instancia para facilitar a busca do otimo conhecido, adicionando a extensão .txt se necessário e removendo espaços em branco
...
# Carrega a instancia do teste Langevin, onde a primeira linha é o numero de nos, seguida por n linhas de matriz de tempo e n linhas de janelas de tempo
...
def rodar_experimentos(pasta_instancias="SolomonPotvinBengio", csv_otimos="otimos_conhecidosSolomon.csv", csv_saida="resultados_experimentosSolomon.csv"):
...
def main():
    if len(sys.argv) == 1:
        ...
        rodar_experimentos("SolomonPotvinBengio", "otimos_conhecidosSolomon.csv", "resultados_experimentosSolomon.csv")
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "--experimentos":
        ...
        pasta_instancias = sys.argv[2] if len(sys.argv) >= 3 else "SolomonPotvinBengio"
        csv_otimos = sys.argv[3] if len(sys.argv) >= 4 else "otimos_conhecidosSolomon.csv"
        csv_saida = sys.argv[4] if len(sys.argv) >= 5 else "resultados_experimentosSolomon.csv"
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "--instancia":
        ...
        try:
            ...
            print("Formato de instancia invalido para o parser Solomon.")
            sys.exit(1)
